...
ObdThread::ObdThread(QtMobility::QBluetoothSocket *bluetoothSocket, QObject *parent) : m_bluetoothSocket(bluetoothSocket), QThread(parent)
{
	...
	m_obd = new obdLib(m_bluetoothSocket);
							...
							*/
							...
							if (pid)
							{
								...
								*/
								...
								emit pidReply(pidreq,done,cycleCounter,(((float)QDateTime::currentDateTime().toTime_t()) + ((float)QDateTime::currentDateTime().time().msec() / 1000.0)));
							}
							else
							{
...
/*
void ObdThread::btReadyRead()
{
	debug("ObdThread::btReadyRead", obdLib::DEBUG_VERBOSE);
	//TODO
	debug("ObdThread::btReadyRead:" << m_bluetoothSocket->bytesAvailable() << " bytes", obdLib::DEBUG_VERBOSE);

    recBuffer += m_bluetoothSocket->readAll();

	while (m_bluetoothSocket->canReadLine()){
		debug("ObdThread::btReadyRead:" << m_bluetoothSocket->readLine(), obdLib::DEBUG_VERBOSE);
	}
}
*/

obdLib *ObdThread::getObdLib(){
    return m_obd;
}
