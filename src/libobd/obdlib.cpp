...
#ifdef QBLUETOOTH
obdLib::obdLib(QtMobility::QBluetoothSocket *bluetoothSocket, QObject *parent): m_btSocket(bluetoothSocket), QObject(parent)
#else
obdLib::obdLib()
#endif
{
...
#elif defined QBLUETOOTH
	len = m_btSocket->write(req, length);
#else
...
#elif defined QBLUETOOTH
	//
	m_btSocket->waitForReadyRead(sleeptime * 1000);
#else
...
#elif defined QBLUETOOTH
		//use signal readyRead? difficult to wait for this
		len = m_btSocket->read(tmp,1024);
#else
		len = read(portHandle,tmp,1024);
#endif
...
#ifdef QBLUETOOTH
//not use now, difficult to handle data
/*
void obdLib::btReadyRead()
{
	debug(obdLib::DEBUG_VERBOSE, "ObdThread::btReadyRead");
	//TODO
	debug("ObdThread::btReadyRead:" << bluetoothSocket->bytesAvailable() << " bytes", obdLib::DEBUG_VERBOSE);

    recBuffer += bluetoothSocket->readAll();

	while (bluetoothSocket->canReadLine()){
		debug(obdLib::DEBUG_VERBOSE, "ObdThread::btReadyRead:" << bluetoothSocket->readLine());
	}
}
*/
#endif
